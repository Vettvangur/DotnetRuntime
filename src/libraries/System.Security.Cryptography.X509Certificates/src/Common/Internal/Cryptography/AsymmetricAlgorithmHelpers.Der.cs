...
namespace Net5.Internal.Cryptography
{
    ...
    internal static partial class AsymmetricAlgorithmHelpers
    {
        ...
        internal static int GetMaxDerSignatureSize(int fieldSizeBits)
        {
            ...
            int segmentSize = 2 + GetDerLengthLength_Local(paddedFieldSizeBytes) + paddedFieldSizeBytes;
            int payloadSize = 2 * segmentSize;
            int sequenceSize = 2 + GetDerLengthLength_Local(payloadSize) + payloadSize;
            return sequenceSize;
        }

        static int GetDerLengthLength_Local(int payloadLength)
        {
