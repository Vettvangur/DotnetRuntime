...
namespace Net5.System.Security.Cryptography.Asn1
{
    ...
    internal partial struct GeneralNameAsn
    {
        ...
        internal void Encode(AsnWriter writer)
        {
            ...
            if (!OtherName.Value.IsEmpty)
            {
                if (wroteValue)
                    throw new CryptographicException();

                OtherName.Encode(writer, new Asn1Tag(TagClass.ContextSpecific, 0));
                wroteValue = true;
            }
            ...
            if (!X400Address.IsEmpty)
            {
                ...
                {
                    if (!Asn1Tag.TryDecode(X400Address.Span, out Asn1Tag validateTag, out _) ||
                ...
                try
                {
                    writer.WriteEncodedValue(X400Address.Span);
                }
            ...
            if (!DirectoryName.IsEmpty)
            {
                ...
                try
                {
                    writer.WriteEncodedValue(DirectoryName.Span);
                }
                catch (ArgumentException e)
                {
                    throw new CryptographicException(SR.Cryptography_Der_Invalid_Encoding, e);
